...
import com.orange.impl.service.enocean.basedriver.EnOceanPacketListener;
...
public class EnOceanHostImpl extends Thread implements EnOceanHost {
	...
	public EnOceanHostImpl(String path, BundleContext bc) {
		...
		try {
			...
			Logger.w(TAG,
					"could not allocate Config Admin chipId/servicePID mapping : "
							+ e.getMessage());
		}
	}
		...
		if (chipId == -1) {
			...
			if (size < MAX_ALLOCATED_CHIP_ID) {
				...
				throw new ArrayIndexOutOfBoundsException(
						"No more CHIP_ID can be allocated.");
			}
		}
	}
	 ...
	 *            an object implementing the {@link EnOceanPacketListener}
	 *            interface.
	 */
	...
	protected void dispatchToListeners(byte[] data) {
		for (int i = 0; i < listeners.size(); i++) {
			EnOceanPacketListener listener = (EnOceanPacketListener) listeners
					.get(i);
	...
	protected EspPacket readPacket() throws IOException {
		...
		Logger.d(
				TAG,
				"header_crc = 0x"
						+ Utils.bytesToHexString(new byte[] { (byte) headerCrc }));
		Logger.d(
				TAG,
				"h_comp_crc = 0x"
						+ Utils.bytesToHexString(new byte[] { Utils
								.crc8(header) }));
		...
		Logger.d(
				TAG,
				"Received EnOcean packet. Frame data: "
						+ Utils.bytesToHexString(Utils.byteConcat(header,
								payload)));
	...
	class ChipPIDMapping {
		...
		public ChipPIDMapping(BundleContext bc) throws UnknownServiceException,
				IOException {
			ServiceReference ref = bc
					.getServiceReference(ConfigurationAdmin.class.getName());
			if (ref != null) {
				ConfigurationAdmin configAdmin = (ConfigurationAdmin) bc
						.getService(ref);
				config = configAdmin
						.getConfiguration(EnOceanBaseDriver.CONFIG_EXPORTED_PID_TABLE);
				...
				throw new UnknownServiceException(
						"ConfigAdmin service was not found !");
			}
		...
		public void put(String servicePID, String chipId) {
			...
			try {
				...
				Logger.w(
						...
						"chip ID '"
								+ chipId
								+ "' could not be saved in ConfigAdmin with service PID '"
								+ servicePID + "'");
			}
		}
