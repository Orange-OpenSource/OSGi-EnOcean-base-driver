...
public class Activator implements BundleActivator, ServiceTrackerCustomizer,
	...
	public void start(BundleContext bundleContext)
		...
		Logger.d(TAG,
				"IN: com.orange.sample.enocean.client.Activator.start(bc: "
						+ bundleContext + ")");
		...
		if (srs == null) {
			...
			while (i < srs.length) {
				...
				Logger.d(TAG,
						"eod.getEncryptionKey(): " + eod.getEncryptionKey());
				Logger.d(TAG,
						"eod.getLearnedDevices(): " + eod.getLearnedDevices());
		...
		Logger.d(TAG,
				"OUT: com.orange.sample.enocean.client.Activator.start(bc: "
						+ bc + ")");
	}
	...
	public Object addingService(ServiceReference reference) {
		...
		if (service != null) {
			if (service instanceof EnOceanDevice) {
				...
				Logger.d(
						TAG,
						"> Registered a new EnOceanDevice : "
								+ Utils.printUid(device.getChipId())
								+ ", device: " + device);
				return service;
			}
		}
	...
	public void modifiedService(ServiceReference reference, Object service) {
		Logger.d(TAG, "> modifiedService(reference: " + reference
				+ ", service: " + service + ")");
		if (service != null) {
			if (service instanceof EnOceanDevice) {
				EnOceanDevice device = (EnOceanDevice) service;
				Logger.d(TAG, "> modifiedService method. device.getChipId(): "
	...
	public void removedService(ServiceReference reference, Object service) {
		Logger.d(TAG, "> removedService(reference: " + reference
				+ ", service: " + service + ")");
		if (service != null) {
			if (service instanceof EnOceanDevice) {
				EnOceanDevice device = (EnOceanDevice) service;
				Logger.d(TAG, "> removedService method. device.getChipId(): "
						+ Utils.printUid(device.getChipId()) + ", device: "
						+ device);
			}
		}
	...
	public void handleEvent(Event event) {
		...
		int i = 0;
		while (i < pns.length) {
			Logger.d(TAG, "pns[" + i + "]: " + pns[i] + ", event.getProperty("
		...
		if (topic.equals(EnOceanEvent.TOPIC_MSG_RECEIVED)) {
			...
			Logger.d(TAG, "> MSG_RECEIVED event : sender=" + displayId
					+ ", profile = '" + profile + "'");
			Logger.d(
					TAG,
			...
			if ("165".equals(rorg)) {
				...
				if ("2".equals(func)) {
					if ("5".equals(type)) {
						Logger.d(TAG,
								"This event has been sent by an A5-02-05 device.");
						Logger.d(
								TAG,
						...
						Logger.d(TAG, "rawTemperatureDB1InNumberAsADouble: "
						...
						if (rawTemperatureDB1InNumberAsADouble < 0) {
							...
							Logger.d(TAG,
									"rawTemperatureDB1InNumberAsADouble is positive, everything is ok.");
						}
						...
						Logger.d(TAG,
								"This event has NOT been sent by an A5-02-05 device. TYPE is NOT equal to 5.");
					}
				} else {
					Logger.d(TAG,
							"This event has NOT been sent by an A5-02-05 device. FUNC is NOT equal to 2.");
				}
				...
				Logger.d(TAG, "payload: " + payload + ", payload.length: "
						+ payload.length);
				int j = 0;
				while (j < payload.length) {
					Logger.d(
				...
				if ("'0x018c0874'".equals(displayId)) {
					...
					Logger.d(TAG,
							"This is the Eltako FRW-WS smoke detector with id 0x018c0874");
					// cf.
					...
					if ((new Integer(0x00).byteValue() & 0xff) == dataDB0InHexAsAnInt) {
						...
						Logger.d(TAG,
								"The given Data DB_0 is UNKNOWN; its value is: "
										+ dataDB0InHexAsAnInt);
					}
					...
					Logger.d(TAG,
							"This is the AfrisoLab Watersensor eco with id: "
									+ displayId);
					if ((new Integer(0x11).byteValue() & 0xff) == dataDB0InHexAsAnInt) {
						if ((new Integer(0x20).byteValue() & 0xff) == statusInHexAsAsAnInt) {
							...
							Logger.d(TAG,
									"The given Data DB_6 is UNKNOWN; its value is: "
					...
					if ((new Integer(0x30).byteValue() & 0xff) == statusInHexAsAsAnInt) {
						...
						Logger.d(TAG,
								"Here, a button has been released (normally, this button was the pressed one.)");
						if (alarmIsActive) {
							// The switch is used to simulate the smoke
							// detector.
							// No Alarm
